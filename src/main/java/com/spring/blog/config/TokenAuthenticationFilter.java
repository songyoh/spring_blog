...
public class TokenAuthenticationFilter extends OncePerRequestFilter {
    ...
    private final static String TOKEN_PREFIX = "Bearer "; // Bearer(띄어쓰기) 확인!

    @Override
    protected void doFilterInternal(HttpServletRequest request,
                                    HttpServletResponse response,
                                    FilterChain filterChain) throws ServletException, IOException {
        // 토큰은 요청헤더를 이용해 제출되기 때문에, Request에서 header를 얻어와야 한다.
        String authorizationHeader = request.getHeader(HEADER_AUTHORIZATION);
        // 얻어온 토큰에서 Bearer 접두사를 삭제하고 토큰값만 얻어내기
        String token = getAccessToken(authorizationHeader); // 토큰값 발라내는 로직은 아래에 메서드로 분리

        if(tokenProvider.validToken(token)){ // 얻어온 토큰이 유효한 토큰이라면
    ...
    private String getAccessToken(String authorizationHeader){
        ...
        if(authorizationHeader != null && authorizationHeader.startsWith(TOKEN_PREFIX)){
            return authorizationHeader.substring(TOKEN_PREFIX.length()); // 앞 7글자(Bearer )를 소거한 값만 리턴되도록
        }
        return null;
    }
}
