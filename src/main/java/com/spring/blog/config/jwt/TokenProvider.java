...
public class TokenProvider {
    ...
    public String generateToken(User user, Duration expiredAt){
        // 아래 makeToken메서드 작성 완료되면 추가로 작성
        Date now = new Date();
        return makeToken(new Date(now.getTime() + expiredAt.toMillis()), user);
    }

    private String makeToken(Date expiry, User user){ // 로그인 성공시
        Date now = new Date(); // 현재 시간으로 토큰 발급 날짜 저장할 준비

        return Jwts.builder() // 빌더패턴으로 토근 생성
                .setHeaderParam(Header.TYPE, Header.JWT_TYPE) // 헤더타입은 JWT (key값, value값)
                .setIssuer(jwtProperties.getIssuer()) // application.properties에 기입된 내용 자동 전달
                ...
                .compact(); // 생성메서드는 빌더패턴이지만 .build()가 아닌 .compact();로 마무리
    }

    // 검증로직은 유효 여부만 리턴하면 되므로 boolean을 리턴한다.
    // 발급된 토큰을 입력받았을 때, 유효하면 true, 유효하지 않으면 false를 리턴
    public boolean validToken(String token){
        try{
            Jwts.parser() // 토큰을 먼저 파싱(읽어들임)하고
                    .setSigningKey(jwtProperties.getSecretKey()) // 복호화시에는 암호화때 사용한 비밀키 입력
                    .parseClaimsJws(token); // 검증 타겟이 되는 토큰 입력
            return true;
        }catch (Exception e){
            return false; // 파싱 및 복호화 비교중 문제가 발생하면 예외가 발생하므로, false 리턴
        }
    }
